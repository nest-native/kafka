/**
 * Audits the *published* supply-chain surface of `@nest-native/kafka`.
 *
 ...
 * production closure (plus any peer deps it chooses to add, such as the
 * optional `@confluentinc/kafka-javascript` client). Auditing the monorepo
 * root instead would flag advisories that live exclusively in the
 * dev/peer/sample tree — none of which can reach a consumer. `npm audit
...
const npmCache = fs.mkdtempSync(path.join(os.tmpdir(), 'nest-native-kafka-audit-cache-'));
const consumerDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nest-native-kafka-audit-consumer-'));
...
try {
  ...
  npm(['run', 'build', '--workspace', '@nest-native/kafka'], { cwd: repoRoot, stdio: 'inherit' });

  const packOutput = npm(['pack', '--json', '--workspace', '@nest-native/kafka'], {
    cwd: repoRoot,
  });
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: 'nest-native-kafka-audit-consumer', version: '0.0.0', private: true }, null, 2)}\n`,
  );
